...
$probePath = Join-Path $root 'observability\scenarios\Test-SocRule100103Rehearsal.ps1'
...
foreach ($contract in @(
    ...
    @{Pattern='for \(\$iteration = 1; \$iteration -le 3; \$iteration\+\+\)[\s\S]*?Start-SocLab\.ps1[\s\S]*?-Scope full[\s\S]*?-ResponseMode observe_only[\s\S]*?Test-SocRule100103Rehearsal\.ps1[\s\S]*?Stop-SocLab\.ps1';Message='Three-TAKE rehearsal does not isolate Full Start, test, and Stop across three sessions.'},
...
if ($probeText -notmatch 'sessionScope[\s\S]*?full[\s\S]*?Full READY observe-only session') {
    throw 'Rule 100103 rehearsal can accidentally reuse a Detection-only session.'
}
