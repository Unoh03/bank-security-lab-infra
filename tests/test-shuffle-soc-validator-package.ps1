...
if ($buildText -notmatch "ConfirmBuild\s+-cne\s+'BUILD SHUFFLE VALIDATOR'" -or
    ...
    throw 'The Shuffle Validator package builder lacks its confirmation, tests, or ZIP step.'
}
if ($uploadText -notmatch "ConfirmUpload\s+-cne\s+'UPLOAD SHUFFLE VALIDATOR'" -or
    ...
    throw 'The Shuffle Validator uploader violates its fixed approval or secret contract.'
}
