...
| CAPITAL-ONE Karpenter Node Role S3 읽기 | `13_capital_one_validation_getobject.cwli` | 공격 Baseline의 성공 `GetObject` 1행·Role·Key·시간창 확인. hardened 복구 뒤 실패 Event는 미검증 |
| CAPITAL-ONE 정상 접근 대조군 | `14_capital_one_negative_control.cwli` | 정상 `terra-user`의 고정 가짜 Object `GetObject` 1행을 확인했고 Node Role Alarm은 `OK`·상태 시각 불변 |
