...
resource "aws_iam_role_policy_attachment" "primary_efs_csi" {
  provider   = aws.primary
  role       = aws_iam_role.primary_efs_csi.name
...
resource "aws_iam_role_policy_attachment" "dr_efs_csi" {
  provider   = aws.dr
  role       = aws_iam_role.dr_efs_csi.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEFSCSIDriverPolicy"
}
...
resource "aws_efs_mount_target" "primary" {
  ...
  security_groups = [aws_security_group.primary_efs.id]
}
...
resource "aws_security_group" "dr_efs" {
  ...
  ingress {
    ...
    to_port         = 2049
    protocol        = "tcp"
    security_groups = var.enable_dr_compute ? [module.dr_eks[0].node_security_group_id] : []
  }
...
resource "aws_efs_mount_target" "dr" {
  provider = aws.dr
  for_each = var.enable_dr_compute ? {
    for index, cidr in var.dr_private_subnets : tostring(index) => module.dr_vpc.private_subnets[index]
  } : {}
  file_system_id  = aws_efs_file_system.dr.id
  subnet_id       = each.value
  security_groups = [aws_security_group.dr_efs.id]
}
...
resource "aws_iam_role_policy" "dr_web_s3" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      { Effect = "Allow", Action = ["s3:ListBucket", "s3:GetBucketLocation"], Resource = aws_s3_bucket.dr.arn },
      { Effect = "Allow", Action = ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"], Resource = "${aws_s3_bucket.dr.arn}/web/*" }
    ]
  })
}

resource "aws_eks_pod_identity_association" "dr_web_s3" {
  count           = var.enable_web_s3_pod_identity && var.enable_dr_compute ? 1 : 0
  provider        = aws.dr
